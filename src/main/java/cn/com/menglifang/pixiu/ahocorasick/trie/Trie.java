package cn.com.menglifang.pixiu.ahocorasick.trie;
...
import cn.com.menglifang.pixiu.ahocorasick.emit.DefaultEmitHandler;
import cn.com.menglifang.pixiu.ahocorasick.emit.Emit;
import cn.com.menglifang.pixiu.ahocorasick.emit.EmitHandler;
import cn.com.menglifang.pixiu.ahocorasick.interval.IntervalTree;
import cn.com.menglifang.pixiu.ahocorasick.interval.Intervalable;
import cn.com.menglifang.pixiu.ahocorasick.state.State;
import cn.com.menglifang.pixiu.ahocorasick.token.FragmentToken;
import cn.com.menglifang.pixiu.ahocorasick.token.MatchToken;
import cn.com.menglifang.pixiu.ahocorasick.token.Token;
...
public class Trie {
    ...
    private void addKeyword(String keyword) {
        ...
        currentState.addEmit(trieConfig.isCaseInsensitive()
    ...
    public void parseText(CharSequence text, EmitHandler emitHandler) {
        ...
        for (int position = 0; position < text.length(); position++) {
            ...
            if (storeEmits(position, currentState, emitHandler)
                    && trieConfig.isStopOnHit()) {
                return;
            }
        }
                ...
                if (emitStrs != null && !emitStrs.isEmpty()) {
                    for (String emitStr : emitStrs) {
                        final Emit emit = new Emit(
                                position - emitStr.length() + 1, position,
                                emitStr);
                        if (trieConfig.isOnlyWholeWords()) {
    ...
    private boolean isPartialMatch(CharSequence searchText, Emit emit) {
        return (emit.getStart() != 0 && Character
                .isAlphabetic(searchText.charAt(emit.getStart() - 1)))
                || (emit.getEnd() + 1 != searchText.length() && Character
                        .isAlphabetic(searchText.charAt(emit.getEnd() + 1)));
    }

    private void removePartialMatches(CharSequence searchText,
    ...
    private void removePartialMatchesWhiteSpaceSeparated(
            CharSequence searchText, List<Emit> collectedEmits) {
        long size = searchText.length();
        List<Emit> removeEmits = new ArrayList<Emit>();
        for (Emit emit : collectedEmits) {
            if ((emit.getStart() == 0 || Character
                    .isWhitespace(searchText.charAt(emit.getStart() - 1)))
                    && (emit.getEnd() + 1 == size || Character.isWhitespace(
    ...
    private boolean storeEmits(int position, State currentState,
